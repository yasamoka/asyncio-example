...
class ConnectionHandler(asyncio.Protocol):
  ...
  def connection_made(self, transport):
    ...
    self.client_username = sock.getpeercert()["subject"][5][0][1]
    self.connectivity_manager.client_connected(self.client_username, self)

  def connection_lost(self, exc):
    self.connectivity_manager.client_disconnected(self.client_username, exc)
  ...
  def data_received(self, data):
    ...
    if self.message_length_remaining == 0:
      self.connectivity_manager.message_received(self.client_username, self.message)
